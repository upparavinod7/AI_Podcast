...
async function finalizeRecording(sessionId) {
  ...
  const concatenatedPath = path.join(sessionDir, "combined-input.webm");
  ...
  try {
    const outputStream = fs.createWriteStream(concatenatedPath);
...
function getFinalRecording(sessionId) {
  ...
  const fileName = `final-host-${sessionId}.webm`;

  const filePath = path.join(FINAL_RECORDINGS_ROOT, fileName);
  ...
  if (stats.size === 0) {
    const error = new Error("Final recording file is empty");
