...
function buildFilterGraph(cues) {
  // Speech audio cleanup filter chain for host:
  // - highpass=f=80: filters out sub-80Hz low-frequency mic rumble/hum
  // - lowpass=f=12000: attenuates high-frequency noise/hiss above 12kHz
  // - aformat: converts to 48kHz mono
  // - volume=0.8: balances loudness with co-host cues while avoiding clipping
  const hostFilter =
    "[0:a]highpass=f=80,lowpass=f=12000,aformat=sample_rates=48000:channel_layouts=mono,volume=0.8[host]";

  if (!Array.isArray(cues) || cues.length === 0) {
    return `${hostFilter};[host]alimiter=limit=0.95:level=1[mixed]`;
  }

  const filters = [hostFilter];
...
function generateTranscript(cues, hostMetadata) {
  const transcript = [];

  // Host entry
  const hostDurationMs = Math.round(
    Number(hostMetadata?.durationSeconds || 0) * 1000,
  );
  transcript.push({
    speaker: "Host",
    voice: null,
    text: "Host transcript is not available.",
    startMs: 0,
    endMs: hostDurationMs,
    cueId: "host-track",
  });
  ...
  cues.forEach((cue) => {
    ...
    const durationMs = Number(cue.actualDurationMs || 0);
    const endMs = startMs + durationMs;

    let voice = cue.voice;
    if (!voice && cue.audioFile) {
      const match = cue.audioFile.match(/-(alex|leo|maaya)\.wav$/i);
      if (match) {
    ...
    const speaker =
      cue.speaker ||
      (voice === "leo" ? "Leo" : voice === "maaya" ? "Maaya" : "Alex");
    ...
    transcript.push({
      ...
      startMs,
      endMs,
      cueId: cue.eventId || `cue-${cue.cueIndex ?? transcript.length}`,
    });
  });
...
function getMixedFilePaths(sessionId) {
  ...
  return {
    ...
    transcriptFileName: `transcript-${sessionId}.json`,
...
async function mixSessionAudio(sessionId, payload) {
  ...
  try {
    ...
    const transcript = generateTranscript(cues, hostMetadata);
    fs.writeFileSync(
      output.transcriptFilePath,
...
function getSessionTranscript(sessionId) {
  ...
  if (!fs.existsSync(output.transcriptFilePath)) {
    throw createMixError(
      "Transcript not found. Finalize and mix the session first.",
      "TRANSCRIPT_NOT_FOUND",
    );
  }

  try {
    const raw = fs.readFileSync(output.transcriptFilePath, "utf8");
    return JSON.parse(raw);
  } catch (parseError) {
    throw createMixError(
      "Failed to parse session transcript",
      "INVALID_TRANSCRIPT",
    );
  }
}
